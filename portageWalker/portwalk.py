...
@param log-level: Set the log level
@param language: To change from the defaule 'en'
@param directory: In the unlikely event that the portage directory is not in /usr/portage
@param database: The name of the database to use
@param dbDir: Location of the database
@param generate: Boolean generate the database
@homepage: Show the hompage infor from the ebuild
...
def gui_main():
    # Use the much safer curses wrapper
    curses.wrapper(draw_window)


def get_config_values(iDir):
    '''
    returns the values from the config file

    @params iDir: The directory where portwalk is installed
    @return log_level: log level
    @return language: The language to search for in ebuilds
    @return portDir: The portage directory in the unlikely event that it's not /usr/portage
    @return hp:  Boolean value for displaying the homepage
    @return name:  Database name 
    @return location: Location of the database
    @return newDB: Boolean if a new database is required
    '''
...
def get_cli_arguments(lvl, lang, dir1, db, dbDir, newDB, hp):
    ...
    @param lvl - log level from configs
    @param lang - The language to search for in ebuilds
    @param dir1 - The portage directory
    @param db - The database name
    @param dbDir - The location of thge database
    @param newDB - Boolean: Create a new database?
    @param hp - Boolean: Display homepage info
