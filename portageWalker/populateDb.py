...
def getDescription(packageDir, packageName):
...
def populatePackages(conn, c, portDir):
    listOfPackages = []
    ...
    for category in listOfCategories:
        ...
        for entry in os.listdir(path):
            if (os.path.isdir(os.path.join(path, entry))):
                description = getDescription(path,entry)
...
if __name__ == "__main__":
    ...
    populateTLD(con, cur, packageGroupDict) 

    # Now populate the packages table
    print("Populatong packages table...")
    populatePackages(con, cur, portageDirectory)
